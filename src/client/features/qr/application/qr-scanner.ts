...
export async function startQrScanner(
  ...
  if (!window.isSecureContext) {
    showHomeError(
      'La camara requiere HTTPS o localhost. Tambien puedes abrir el QR con la app Camara del movil.',
    )
    return
  }
  ...
  try {
    ...
    const scan = async () => {
      ...
      try {
        ...
        if (codes[0]?.rawValue)
          dom.scannerStatus.textContent = 'QR detectado, pero no parece una sala de fAir Drop.'
      } catch {
