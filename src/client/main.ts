...
const isMobileDevice = matchMedia('(pointer: coarse)').matches || /mobile|android|iphone|ipad/i.test(navigator.userAgent);
...
function bindEvents(): void {
  ...
  dom.btnCopyCode.addEventListener('click', () => copyButtonText(dom.btnCopyCode, dom.roomCodeDisplay.textContent ?? '', 'Copiar'));
  dom.btnCopyLink.addEventListener('click', () => copyButtonText(dom.btnCopyLink, dom.shareUrl.textContent ?? '', 'Copiar link'));
...
async function copyButtonText(button: HTMLButtonElement, text: string, original: string): Promise<void> {
  if (!text) return;
