...
app.get('/api/status', (req, res) => {
  const data = {
    ...
    bans: [...bans.entries()].map(([ip, b]) => ({
      ip, type: b.until ? 'temporal' : 'permanente', label: banLabel(b),
    })),
    rooms_detail: [],
  };
...
wss.on('connection', (ws, req) => {
  ...
  clients.set(ws, { ip, userAgent, connectedAt: new Date() });
  ...
  ws.on('message', (data, isBinary) => {
    ...
    switch (msg.type) {
      ...
      case 'join-room': {
        ...
        if (!room) { send(ws, { type: 'error', message: 'Sala no encontrada' }); return; }
        ...
        // Enviar info mutua a ambos clientes
      ...
      case 'ban-peer': {
        ...
        setTimeout(() => info.peer.close(), 400);
        console.log(`[Ban] ${peerMeta.ip} baneado — ${until ? `hasta ${until.toISOString()}` : 'permanente'}`);
        break;
      }
  ...
  ws.on('close', () => {
    ...
    clients.delete(ws);
    if (!info) return;
    const { code, room, role, peer } = info;
